 ...
 * 2. Preencha todos os campos obrigatórios (id, slug, name, category, price…).
 * 3. Para imagens: coloque os arquivos em /public/images/products/<slug>/
 *    e liste os caminhos no array `images[]`.
 * 4. O slug deve ser único e em kebab-case (ex: "quadro-abstrato-aurora").
 * 5. Salve o arquivo — o site atualiza automaticamente.
 *
 * MIGRAÇÃO PARA CMS (Sanity / Contentful / Strapi):
 * - A interface `Product` abaixo define o contrato de dados.
 * - Ao migrar, substitua o array `products` por uma chamada à API do CMS,
 *   retornando objetos que obedeçam a mesma interface.
 * - As funções utilitárias (getAll, getBySlug, getByCategory…) continuam
 *   funcionando sem alterar os componentes.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
export interface ProductDimensions {
  width: number   // cm
  height: number  // cm
  depth?: number  // cm (espessura, opcional)
}
...
export interface Product {
  ...
  price: number                 // Preço em reais (número inteiro)
  priceInstallments?: number    // Número de parcelas sem juros (padrão: 12)
  ...
  fullDescription: string       // Descrição completa para página do produto
  images: string[]              // Caminhos das imagens em /public/
  thumbnailColor: string[]      // [cor1, cor2] — gradiente placeholder
  badge?: string                // Etiqueta especial: "Novo", "Destaque", etc.
  featured: boolean             // Aparece na home
  available: boolean            // Se está disponível para venda
  deliveryDays: string          // Prazo de entrega estimado
  createdAt: string             // ISO date — usado para ordenação "mais recentes"
}

// ─── DADOS MOCK ─────────────────────────────────────────────────────────────
// Substitua os thumbnailColor por imagens reais em /public/images/products/<slug>/
// Convenção de nomes: 01.jpg, 02.jpg, 03.jpg (principal + secundárias)
...
export const products: Product[] = [
  ...
  },
  {
    ...
    finish: ['verniz-fosco'],
    description:
      'Pintura em óleo com técnica de espátula que recria a textura dos céus de inverno. Peça única e original.',
    fullDescription:
  ...
  },
  {
    ...
    finish: ['polido'],
    description:
      'Fotografia artística da arquitetura modernista de Brasília. Homenagem ao design brasileiro em alta definição.',
    fullDescription:
      'Uma homenagem à genialidade de Niemeyer e ao modernismo brasileiro. Esta fotografia captura a geometria sublime da capital federal em luz de fim de tarde, imprimindo em substrato metalizado que adiciona profundidade e contraste únicos. O vidro flotado antirreflexo garante visibilidade perfeita em qualquer iluminação. A moldura slim de alumínio polido não concorre com a imagem — apenas a enquadra com precisão cirúrgica.',
  ...
  // ── MOLDURAS ─────────────────────────────────────────────────────────────
  {
    ...
    description:
      'Moldura oval com acabamento dourado champagne. Elegância atemporal para fotografias e obras de arte.',
    fullDescription:
      'A Champagne Oval resgata a tradição das molduras ovais europeias com um olhar contemporâneo. O MDF de alta densidade é esculpido em perfil clássico e banhado em folha de cobre champagne aplicada à mão, camada por camada. O vidro antirreflexo incluso protege a obra sem interferir na apreciação. Disponível sob medida — consulte nosso atendimento para dimensões personalizadas.',
  ...
  },
  {
    ...
    description:
      'Linha minimalista em madeira escura. Perfeita para fotografias e obras modernas sem concorrer com o conteúdo.',
    fullDescription:
      'Ébano Slim é o essencial levado à perfeição. O perfil de apenas 8mm em madeira maciça ebonizada desaparece elegantemente ao redor da obra, dando protagonismo total ao conteúdo emoldurado. O processo de ebonização é natural — sem tinta, apenas tratamento químico que realça as fibras da madeira. Vidro flotado antirreflexo de 2mm incluso. Disponível nos tamanhos padrão e sob medida.',
  ...
  },
  {
    ...
    finish: ['envelhecido'],
    description:
      'Moldura estilo provençal com detalhes esculpidos à mão e pintura giz envelhecida. Charme e delicadeza únicos.',
    fullDescription:
      'Inspirada nas casas de campo do sul da França, a Provençal Branca carrega toda a delicadeza do estilo shabby chic refinado. Os detalhes florais são esculpidos à mão em MDF de alta densidade, e a pintura giz é aplicada em camadas — depois parcialmente lixada nas extremidades, criando o efeito de envelhecimento natural. Finalizada com cera de abelha que protege e adiciona leve brilho acetinado.',
  ...
  },
  {
    ...
    description:
      'Moldura com acabamento em bronze envelhecido, detalhes em relevo e pátina artesanal. Elegância intemporal.',
    fullDescription:
      'A Bronze Clássica é uma declaração de amor ao artesanato tradicional. Fundida em resina de alta densidade e banhada em bronze real, cada peça passa pelo processo artesanal de pátina — uma oxidação controlada que cria veios e tonalidades únicas. Os detalhes em relevo incluem folhas de acanto e pérolas que circulam o perfil externo. Uma peça que vale tanto pela moldura quanto pelo que emoldura.',
  ...
  // ── ESPELHOS ─────────────────────────────────────────────────────────────
  {
    ...
    description:
      'Espelho de arco com moldura elaborada em detalhes florais. Peça de destaque para salas e quartos sofisticados.',
    fullDescription:
      'O Arco Florentino é uma relíquia contemporânea. Inspirado nos espelhos venezianos do século XVIII, apresenta moldura em resina de alta fidelidade com folha de ouro 22 quilates aplicada artesanalmente. O espelho em si é bisotado — o chanfro perimetral de 45° cria um efeito de luz que enriquece qualquer ambiente. Acompanha suporte de parede oculto, hardware de fixação e instruções detalhadas.',
  ...
  },
  {
    ...
    description:
      'Estilo industrial sofisticado com detalhes em ferro forjado ennegrecido. Charme vintage para ambientes contemporâneos.',
    fullDescription:
      'O Bistro Vintage conjura a atmosfera dos cafés parisienses do início do século XX — mas com vocabulário absolutamente atual. A moldura em ferro forjado artesanalmente combina barra chata e fio quadrado em composição geométrica que enquadra um espelho bisotado de efeito levemente antiquado. A pátina escura do ferro foi aplicada com calor — não com tinta — garantindo durabilidade sem manutenção.',
  ...
  },
  {
    ...
    description:
      'Design inspirado no Art Déco com formas geométricas em latão polido. Uma obra de arte funcional.',
    fullDescription:
      'O Art Déco Geométrico é um manifesto estético. Cada linha da moldura de latão polido foi pensada para evocar a geometria pura do movimento Art Déco — ângulos, raios e proporções que dialogam com a arquitetura de interiores de alto padrão. A estrutura interna em aço inox garante que o latão não deforme com variações de temperatura. O espelho bisotado de 6mm completa a peça com chanfro preciso. Inclui projeto de instalação.',
  ...
  },
  {
    ...
    description:
      'Espelho de corpo inteiro com moldura laqueada em dourado. Peça de destaque para closets, halls e suítes premium.',
    fullDescription:
      'Grand Miroir é um espelho que redefine o espaço onde é instalado. Com 180cm de altura, oferece visão de corpo inteiro com reflexo de excepcional clareza óptica. A moldura em madeira maciça recebe aplicação de laca em camadas — cada uma lixada e polida — antes do acabamento dourado final. O resultado é uma superfície que rivaliza com espelhos de manufatura europeia do século XIX. Suporte de parede com regulagem de inclinação incluso.',
...
// ─── FUNÇÕES UTILITÁRIAS ─────────────────────────────────────────────────────
// Estas funções são o ponto único de acesso aos dados.
// Ao migrar para um CMS, apenas estas funções precisam ser alteradas.
...
export function formatDimensions(dim: ProductDimensions): string {
  ...
  if (dim.depth) parts.push(`${dim.depth}cm de profundidade`)
  return parts.join(' · ')
}
